...
// --- Native Banner Component (Hardcoded invokeKey) ---
// This component now takes only 'adKey' from ads.json
function AdsterraNativeBanner({ adKey }) {
  const adContainerRef = useRef(null);
  
  const containerId = `container-${adKey}`; 
  
  // --- This 'invokeKey' is hardcoded as you requested ---
...
// --- Popup Component ---
// This also uses the new AdsterraNativeBanner component
...
export default function HomePageClient({ 
  ...
  // --- ✅✅✅ NEW IN-FEED AD LOGIC (START) ---
  // Get the array of keys from adSettings (it comes from ads.json)
  const inFeedKeys = adSettings.homepageInFeedAdKeys || [];
  
  // Check if ads should be shown at all
  const showInFeedAds = showAds && 
                        adSettings.showHomepageInFeedAds && 
                        inFeedKeys.length > 0; // Only show if at least one key exists
  // --- ✅✅✅ NEW IN-FEED AD LOGIC (END) ---
  ...
  return (
    <main>
      ...
      {showBannerAd && (
        // This now uses the 'homepageBannerAdKey' from ads.json
        <AdsterraNativeBanner 
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            {filtered.map((p, i) => (
              <Fragment key={p.id || p.name}>
                ...
                {/* --- ✅✅✅ UPDATED IN-FEED AD LOGIC ---
                   This logic (i % 2 === 1) is correct and shows an ad after every 2 products (i.e., every line).
                   It now implements your requested key rotation logic.
                --- */}
                {showInFeedAds && (i % 2 === 1) && (
                  <div className="col-span-2 md:grid-cols-3 lg:grid-cols-4" key={`ad-${i}`}>
                    <AdsterraNativeBanner 
                      adKey={
                        inFeedKeys.length === 1
                          // 1. If only one key, always use it
                          ? inFeedKeys[0] 
                          // 2. If multiple keys, rotate them based on the ad slot index
                          : inFeedKeys[ Math.floor(i / 2) % inFeedKeys.length ]
                      } 
                    />
                  </div>
                )}
      ...
      {showPopup && (
        // This now uses the 'homepagePopupAdKey' from ads.json
        <PopupAd 
