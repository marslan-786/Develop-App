// --- 3. app/HomePageClient.js ---
// (یہاں سے responsive کلاسز ہٹائی گئی ہیں)
          ...
          <button
            ...
            className={`px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-300
              ${activeFilter === filter.id 
                ? 'animated-gradient-button text-white shadow-lg' 
...
function FloatingWhatsAppButton({ whatsappNumber }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="fixed bottom-6 right-6 z-20 p-6 bg-green-500 text-white rounded-full shadow-lg transition-transform hover:scale-110"
    >
      <IconWhatsApp />
    </a>
  );
