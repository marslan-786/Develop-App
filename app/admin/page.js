...
export default async function AdminPage({ searchParams }) {
  ...
  if (await isValidPassword(passwordQuery)) {
    ...
    // --- ✅ تبدیلی: بینر URL بھی لوڈ کریں ---
    let bannerUrl = ""; // ڈیفالٹ خالی
    try {
      const bannerBlob = await head('background.png', { cache: 'no-store' });
      bannerUrl = bannerBlob.url;
    } catch (error) { /* اگر بینر نہ ملے تو کوئی بات نہیں */ }
    // --- --- ---
    
    return (
      <AdminDashboardClient 
        logoUrl={logoUrl} 
        bannerUrl={bannerUrl} // <-- بینر کو یہاں پاس کریں
        passwordQuery={passwordQuery} 
      >
        {/* --- یہ آپ کا 3-بٹن والا ڈیش بورڈ ہے (اسے چھیڑنے کی ضرورت نہیں) --- */}
